  ...
  return (
    <TabsContent value="tools" className="space-y-4">
      <div>
        ...
        {tools.length === 0 ? (
          <p className="text-sm text-muted-foreground italic">No tools available. Click "List Tools" to load them.</p>
        ) : (
                            ...
                            {schema.type === 'string' && schema.enum ? (
                              <select
                                ...
                                onChange={(e) =>
                                  setParams({
                                    ...params,
                                    [index]: { ...toolParams, [key]: e.target.value }
                                  })
                                }
                              >
                              ...
                              <div className="flex items-center space-x-2">
                                <input
                                  ...
                                  onChange={(e) =>
                                    setParams({
                                      ...params,
                                      [index]: { ...toolParams, [key]: e.target.checked }
                                    })
                                  }
                                />
                                <Label htmlFor={`${index}-${key}`}>{schema.description}</Label>
                              </div>
                            ) : (
                              <Textarea
                                id={`${index}-${key}`}
                                ...
                                onChange={(e) => {
                                  try {
                                    setParams({
                                      ...params,
                                      [index]: { ...toolParams, [key]: JSON.parse(e.target.value) }
                                    })
                                  } catch {
                                    setParams({
                                      ...params,
