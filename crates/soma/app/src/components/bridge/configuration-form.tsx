...
export const LinkProviderInstancesTable = ({
	...
	const [unlinkingInstanceId, setUnlinkingInstanceId] = useState<string | null>(null);
	...
	const isAlreadyLinked = (providerInstanceId: string) => {
		...
		return functionInstancesData.items.some(
			(fi) => fi.provider_instance_id === providerInstanceId && fi.function_controller_type_id === functionTypeId
		);
	};
							...
							{instances.map((instance, index) => {
								...
								return (
									<TableRow key={instance.id} index={index}>
										...
										<TableCell>
											{getCredentialControllerName(instance.credential_controller_type_id)}
										</TableCell>
										<TableCell>
											<span className={cn(
										...
										<TableCell className="text-right min-w-40">
											{alreadyLinked ? (
												...
												>
													...
													{unlinkingInstanceId === instance.id ? "Unlinking..." : "Unlink"}
												</Button>
											) : (
												<Button
													variant="outline"
												...
												>
													...
													{linkingInstanceId === instance.id ? "Linking..." : "Link"}
												</Button>
											)}
										</TableCell>
									</TableRow>
...
const AddNewProviderInstance = ({
	...
	const currentCredentialTypeId = existingProviderInstance?.credential_controller_type_id;
		...
		<div className="space-y-4">
			<div>
				...
				<p className="text-sm text-muted-foreground">
					This provider supports multiple authentication methods. {existingProviderInstance ? "The current authentication method is marked with a checkmark." : "Expand each of them below to find out which one suits your needs."}
				</p>
			</div>
			<Accordion type="single" collapsible className="w-full" defaultValue={defaultValue}>
				{credentialControllers.map((credController) => {
					const isCurrentCredential = currentCredentialTypeId === credController.type_id;
					return (
...
const ResourceServerConfigurationForm = ({
	...
	const [accountName, setAccountName] = useState(existingProviderInstance?.display_name || "");
	...
	const shouldPopulateFromExistingResourceServer = existingResourceServerCredential &&
		existingResourceServerCredential.type_id === getExpectedResourceServerTypeId(credentialController.type_id);

	const shouldPopulateFromExistingUserCredential = existingUserCredential &&
		existingUserCredential.type_id === getExpectedUserCredentialTypeId(credentialController.type_id);

	// Helper function to determine expected resource server credential type ID based on credential controller type
	function getExpectedResourceServerTypeId(credentialControllerTypeId: string): string {
		...
		const typeMapping: Record<string, string> = {
			...
			'oauth2_jwt_bearer_assertion_flow': 'oauth2_jwt_bearer_assertion_flow_resource_server',
			// Add more mappings as needed
		};
		return typeMapping[credentialControllerTypeId] || credentialControllerTypeId + '_resource_server';
	}

	// Helper function to determine expected user credential type ID based on credential controller type
	function getExpectedUserCredentialTypeId(credentialControllerTypeId: string): string {
		...
		return typeMapping[credentialControllerTypeId] || credentialControllerTypeId + '_user';
	}
	...
	// Mutations
	const encryptConfigMutation = $api.useMutation("post", "/api/bridge/v1/available-providers/{provider_controller_type_id}/available-credentials/{credential_controller_type_id}/credential/resource-server/encrypt");
	const createCredentialMutation = $api.useMutation("post", "/api/bridge/v1/available-providers/{provider_controller_type_id}/available-credentials/{credential_controller_type_id}/credential/resource-server");
	const createProviderInstanceMutation = $api.useMutation("post", "/api/bridge/v1/available-providers/{provider_controller_type_id}/available-credentials/{credential_controller_type_id}");
	...
	const rawUserCredentialSchema = credentialController.configuration_schema.user_credential;
	...
	const userCredentialSchema = rawUserCredentialSchema ? { ...rawUserCredentialSchema } : null;
	...
	const showUserCredentialForm = !credentialController.requires_brokering && userCredentialSchema;

	// Combine schemas if showing user credential form
	const combinedSchema = showUserCredentialForm && userCredentialSchema
		? {
			...
			required: [
				...(Array.isArray(resourceServerSchema.required) ? resourceServerSchema.required : []),
				...(Array.isArray(userCredentialSchema.required) ? userCredentialSchema.required : []),
			],
		}
		: resourceServerSchema;
	...
	const handleSubmit = async (data: { formData?: unknown }) => {
		...
		try {
			...
			const resourceServerProperties = resourceServerSchema.properties as Record<string, JsonSchema> || {};
			const userCredentialProperties = showUserCredentialForm && userCredentialSchema
			...
			if (shouldPopulateFromExistingResourceServer && existingResourceServerCredential?.value) {
				const existingValue = existingResourceServerCredential.value as Record<string, any>;
				...
				Object.keys(resourceServerProperties).forEach((key) => {
					...
					const isPasswordField = propFormat === "password" ||
					                       key.toLowerCase().includes("secret") ||
					                       key.toLowerCase().includes("password");

					// If this is a password field and it's empty/undefined, use the original value
					if (isPasswordField && !resourceServerData[key] && key in existingValue) {
			...
			if (showUserCredentialForm && shouldPopulateFromExistingUserCredential && existingUserCredential?.value) {
				...
				Object.keys(userCredentialProperties).forEach((key) => {
					...
					// If this is a password field and it's empty/undefined, use the original value
					if (isPasswordField && !userCredentialData[key] && key in existingValue) {
						updatedData[key] = existingValue[key];
					}
				});
			...
			const encryptedResourceServerConfig = await encryptConfigMutation.mutateAsync({
				params: {
					path: {
			...
			const resourceServerCredential = await createCredentialMutation.mutateAsync({
				params: {
					path: {
						provider_controller_type_id: providerTypeId,
						credential_controller_type_id: credentialController.type_id,
			...
			console.log("Resource server credential created:", resourceServerCredential);
			...
			if (showUserCredentialForm) {
				...
				const encryptedUserCredentialConfig = await encryptUserCredentialMutation.mutateAsync({
					params: {
						path: {
							provider_controller_type_id: providerTypeId,
							credential_controller_type_id: credentialController.type_id,
			...
			if (credentialController.requires_brokering && resourceServerCredential.id) {
				...
				if (brokeringResponse && typeof brokeringResponse === "object" && "type" in brokeringResponse) {
					if (brokeringResponse.type === "broker_state" && "action" in brokeringResponse) {
			...
			setFormErrors([
				error instanceof Error ? error.message : "Failed to create credentials. Please try again."
			]);
			setIsSubmitting(false);
		}
	};
	...
	const properties = (combinedSchema.properties as Record<string, JsonSchema>) || {};
	const resourceServerProperties = (resourceServerSchema.properties as Record<string, JsonSchema>) || {};
	const userCredentialProperties = showUserCredentialForm && userCredentialSchema
		? (userCredentialSchema.properties as Record<string, JsonSchema> || {})
		: {};
	...
	if (shouldPopulateFromExistingResourceServer && existingResourceServerCredential?.value) {
		...
		Object.keys(resourceServerProperties).forEach((key) => {
			const prop = resourceServerProperties[key];
			const propFormat = prop.format as string | undefined;
			const isPasswordField = propFormat === "password" || key.toLowerCase().includes("secret") || key.toLowerCase().includes("password");
