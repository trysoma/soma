...
export const Route = createFileRoute('/bridge/manage-credentials/$providerInstanceId')({
  component: RouteComponent,
})

function RouteComponent() {
  const { providerInstanceId } = useParams({ from: '/bridge/manage-credentials/$providerInstanceId' });
  ...
  const tabs = [
    {
      ...
      component: (
        <Link to={LINKS.BRIDGE_MANAGE_CREDENTIALS_DOCUMENTATION(providerInstanceId)}>
    ...
    },
    {
      ...
      component: (
        <Link to={LINKS.BRIDGE_MANAGE_CREDENTIALS_CONFIGURATION(providerInstanceId)}>
    ...
    },
    {
      ...
      component: (
        <Link to={LINKS.BRIDGE_MANAGE_CREDENTIALS_FUNCTIONS(providerInstanceId)}>
          Enabled Functions
        </Link>
      ),
    },
  ...
  return (
    ...
    >
      <SlideOutTabs tabs={tabs} getCurrentTab={getCurrentTab} className="grid-cols-4" />
    </SlideOutPanel>
  );
}
