...
export const Route = createFileRoute('/bridge/enable-functions/available/$functionControllerId/test')({
  component: RouteComponent,
})

function RouteComponent() {
  const { functionControllerId } = useParams({ from: '/bridge/enable-functions/available/$functionControllerId/test' });

  const [selectedProviderInstanceId, setSelectedProviderInstanceId] = useState<string>("");
  ...
  if (enabledInstances.length === 0) {
    return (
      <div className="p-6">
        <p className="text-muted-foreground">No enabled credentials for this function. Please configure credentials first.</p>
      </div>
    );
  }
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="mb-6 max-w-2xl">
        <label htmlFor="provider-instance" className="block text-sm font-medium mb-2">
        ...
        <Select value={selectedProviderInstanceId} onValueChange={setSelectedProviderInstanceId}>
          <SelectTrigger className="w-full bg-white">
            <SelectValue placeholder="Select credentials..." />
          </SelectTrigger>
          <SelectContent>
            {enabledInstances.map((instance) => (
              <SelectItem key={instance.provider_instance.id} value={instance.provider_instance.id}>
