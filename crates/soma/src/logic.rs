...
use crate::{a2a, repository::{CreateTaskTimelineItem, Repository, TaskRepositoryLike, UpdateTaskStatus}};
use shared::{
        ...
        .message_to_connections(
            ...
            a2a_rs::events::Event::TaskStatusUpdate(a2a_rs::types::TaskStatusUpdateEvent {
                ...
                final_: match request.inner.status {
                    TaskStatus::Completed | TaskStatus::Failed | TaskStatus::Canceled | TaskStatus::Rejected => true,
                    _ => false,
                },
...
pub async fn create_message(
    ...
    repository.insert_message(&message.clone().try_into()?).await?;
...
mod tests {
    ...
    async fn test_create_message() {
        ...
        let messages = repo.get_messages_by_task_id(&task.id, &pagination).await.unwrap();
    ...
    async fn test_get_task_timeline_items_with_status_updates() {
        ...
        update_task_status(&repo, &connection_manager, status_request).await.unwrap();
    ...
    async fn test_connection_manager_add_and_remove() {
        ...
        let result = connection_manager.remove_connection(WrappedUuidV4::new(), connection_id.clone());
        assert!(result.is_err());
    }
    ...
    async fn test_connection_manager_message_to_connections() {
        ...
        // Add two connections
        let (_conn_id1, mut receiver1) = connection_manager.add_connection(task_id.clone()).unwrap();
        let (_conn_id2, mut receiver2) = connection_manager.add_connection(task_id.clone()).unwrap();
        ...
        connection_manager.message_to_connections(task_id.clone(), event.clone()).await.unwrap();
