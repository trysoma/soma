...
fn find_free_port(start: u16, end: u16) -> std::io::Result<u16> {
    ...
    Err(std::io::Error::new(std::io::ErrorKind::AddrNotAvailable, "No free ports found"))
}
...
pub async fn cmd_start(subsys: &SubsystemHandle, params: StartParams) -> Result<(), CommonError> {
    ...
    let (db, conn, repository) = setup_repository(params.db_conn_string.clone(), params.db_auth_token.clone()).await?;

    let (kill_restate_signal_trigger, kill_restate_signal_receiver) = oneshot::channel::<()>();
    let (shutdown_complete_restate_signal_trigger, shutdown_complete_restate_signal_receiver) = oneshot::channel::<()>();
...
async fn start_dev_restartable_processes_on_config_change(subsys: &SubsystemHandle, params: DevRestartableProcesses) -> Result<(), CommonError> {

    loop {
        let params_clone = params.clone();
        let mut file_change_rx: broadcast::Receiver<FileChangeEvt> = params_clone.file_change_tx.subscribe();
...
async fn start_dev_restartable_processes(subsys: &SubsystemHandle, params: DevRestartableProcesses, soma_config: SomaConfig) -> Result<(), CommonError> {
    ...
    let (shutdown_runtime_complete_signal_trigger, shutdown_runtime_complete_signal_receiver) = oneshot::channel::<()>();
    let src_dir_clone = src_dir.clone();
    let runtime_clone = runtime.clone();
    ...
    let router_params = router::RouterParams::new(params.clone(), router::InitRouterParams {
        connection_manager,
        repository,
        mcp_transport_tx,
...
fn get_soma_config(src_dir: PathBuf) -> Result<SomaConfig, CommonError> {
    let soma_config = utils::soma_agent_config::SomaConfig::from_yaml(
        &fs::read_to_string(src_dir.join("soma.yaml"))?
    )
...
async fn on_soma_config_change(file_change_rx: &mut FileChangeRx) -> Result<bool, CommonError> {
    loop {
        ...
        if event.changes.iter().any(|change| change.paths.iter().any(|path| path.ends_with("soma.yaml"))) {
            return Ok(true);
        }
    }
}
...
async fn start_restate_deployment(params: StartParams, runtime_port: u16, soma_config: SomaConfig) -> Result<(), CommonError> {
    ...
    info!("Registering service at {} with Restate admin at {}", service_uri, params.restate_admin_url);
...
async fn start_restate_server(kill_signal: oneshot::Receiver<()>, shutdown_complete: oneshot::Sender<()>) -> Result<(), CommonError> {
    ...
    cmd
        ...
        .arg("--tracing-filter")
        .arg("warn");
    run_child_process("restate-server", cmd, Some(kill_signal), Some(shutdown_complete), None).await?;
    Ok(())
}


fn start_axum_server(params: StartParams, router_params: RouterParams) -> Result<(impl Future<Output = Result<(), std::io::Error>>, axum_server::Handle), CommonError> {
...
async fn start_runtime_bun_v1(src_dir: PathBuf, runtime_port: u16, kill_signal: oneshot::Receiver<()>, shutdown_complete: oneshot::Sender<()>) -> Result<(), CommonError> {
    let mut cmd = Command::new("bun");
    cmd.arg("index.ts")
        .current_dir(src_dir);
    run_child_process("bun", cmd, Some(kill_signal), Some(shutdown_complete), Some(HashMap::from([("PORT".to_string(), runtime_port.to_string())]))).await?;
...
fn determine_runtime(params: StartParams) -> Result<Option<Runtime>, CommonError> {
    ...
    let possible_runtimes = vec![
        (Runtime::BunV1, validate_runtime_bun_v1),
    ];
    ...
    for (runtime, validate_fn) in possible_runtimes {
     ...
     match matched_runtimes.len() {
        ...
        _ => return Err(CommonError::Unknown(anyhow::anyhow!("Multiple runtimes matched"))),
    };
...
async fn start_dev_file_watcher(
    ...
    let (file_change_debounced_tx, mut file_change_debounced_rx) = mpsc::channel::<(Instant, Vec<notify::Event>)>(10);
    ...
    tokio::spawn({
        ...
        async move {
            ...
            while let Some((ts, changes)) = file_change_debounced_rx.recv().await {
                ...
                let _ = file_change_tx.send(FileChangeEvt { changes: debounced_changes });
        ...
        match result {
            Ok(events) => {
                ...
                for event in events {
                    ...
                    let ignored = event.event.paths.iter().any(|p| files_to_ignore_clone.is_match(p));
                    if ignored {
                        continue;
                    }
                ...
                if !any_relevant.is_empty() {
                    ...
                    let _ = file_change_debounced_tx.blocking_send((Instant::now(), any_relevant));
                }
            },
...
async fn start_dev_runtime(src_dir: PathBuf, runtime: Runtime, runtime_port: u16, file_change_signal: &mut FileChangeRx, mut kill_signal: broadcast::Receiver<()>, shutdown_complete_signal: oneshot::Sender<()>) -> Result<(), CommonError> {
    ...
    loop {
        ...
        let serve_fut = match runtime {
            Runtime::BunV1 => build_runtime_bun_v1(src_dir.clone()).and_then(|_| start_runtime_bun_v1(src_dir.clone(), runtime_port, dev_kill_signal_rx, dev_shutdown_complete_tx)),
        };
