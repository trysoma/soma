...
mod tests {
    mod integration {
        use super::super::*;
        use crate::logic::sts::config::{DevModeConfig, JwtTemplateModeConfig};
        ...
        use crate::test::token_validation::{
            AccessTokenAssertions, decode_and_validate_access_token,
            decode_and_validate_refresh_token,
        };
        use shared::primitives::WrappedChronoDateTime;
        use shared_macros::integration_test;

        /// Helper to create STS config directly via repository (bypasses auth for testing)
        async fn create_test_sts_config(ctx: &TestContext, config: StsTokenConfig) {
            ...
            ctx.identity_repo
                ...
                .await
                .expect("Failed to create STS config");
        }
        ...
        async fn test_exchange_sts_token_dev_mode() {
            ...
            create_test_sts_config(&ctx, dev_config).await;
        ...
        async fn test_exchange_sts_token_jwt_template_with_dex_tokens() {
            ...
            create_test_sts_config(&ctx, jwt_config).await;
