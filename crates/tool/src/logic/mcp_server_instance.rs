...
pub struct McpServerInstanceToolSerialized {
...
pub struct McpServerInstanceSerializedWithFunctions {
    ...
    pub created_at: WrappedChronoDateTime,
    pub updated_at: WrappedChronoDateTime,
    pub tools: Vec<McpServerInstanceToolSerialized>,
}
...
#[authz_role(Admin, permission = "mcp:write")]
#[authn]
...
#[authz_role(Admin, Maintainer, permission = "mcp:read")]
#[authn]
...
pub async fn add_mcp_server_instance_function_internal<R: ProviderRepositoryLike>(
    ...
    trace!(
        ...
        function_type = %request.tool_deployment_type_id,
    ...
    // Check if tool_name is already used in this instance
    let existing = repository
        .get_mcp_server_instance_tool_by_name(mcp_server_instance_id, &request.tool_name)
        .await?;
    ...
    if existing.is_some() {
        return Err(CommonError::InvalidRequest {
            msg: format!(
                ...
                request.tool_name, mcp_server_instance_id
            ),
            source: None,
        });
    }
    ...
    let tool_instance = repository
        .get_tool_instance_by_id(
            &request.tool_deployment_type_id,
            &request.tool_group_deployment_type_id,
            &request.tool_group_id,
        )
        .await?
        .ok_or_else(|| {
            CommonError::Unknown(anyhow::anyhow!(
    ...
    let create_params = CreateMcpServerInstanceFunction {
        ...
        tool_group_id: tool_instance.tool_group_id.clone(),
    ...
    repository
        .create_mcp_server_instance_tool(&create_params)
        .await?;

    // Create the serialized function for the event
    let function_serialized = McpServerInstanceToolSerialized {
        mcp_server_instance_id: mcp_server_instance_id.to_string(),
        tool_deployment_type_id: tool_instance.tool_deployment_type_id,
        tool_group_deployment_type_id: tool_instance.tool_group_deployment_type_id,
        tool_group_id: tool_instance.tool_group_id,
        tool_name: request.tool_name,
    ...
    if publish_on_change_evt {
        on_config_change_tx
            .send(OnConfigChangeEvt::McpServerInstanceToolAdded(
                function_serialized,
            ))
            .map_err(|e| {
                CommonError::Unknown(anyhow::anyhow!("Failed to send config change event: {e}"))
...
#[allow(clippy::too_many_arguments)]
pub async fn update_mcp_server_instance_tool_internal<R: ProviderRepositoryLike>(
    on_config_change_tx: &OnConfigChangeTx,
    repository: &R,
    mcp_server_instance_id: &str,
    ...
    trace!(
        instance_id = %mcp_server_instance_id,
        function_type = %tool_deployment_type_id,
        provider_type = %tool_group_deployment_type_id,
        tool_group_instance_id = %tool_group_instance_id,
        new_name = %request.tool_name,
    ...
    let existing_function = instance.tools.iter().find(|f| {
        f.tool_deployment_type_id == tool_deployment_type_id
            && f.tool_group_deployment_type_id == tool_group_deployment_type_id
            && f.tool_group_id == tool_group_instance_id
    });

    let existing_function = existing_function.ok_or_else(|| {
        CommonError::Unknown(anyhow::anyhow!(
            "Function not found in MCP server instance: {mcp_server_instance_id}/{tool_deployment_type_id}/{tool_group_deployment_type_id}/{tool_group_instance_id}"
        ))
    })?;

    // Check if new tool_name conflicts with another function in this instance
    let existing = repository
        .get_mcp_server_instance_tool_by_name(mcp_server_instance_id, &request.tool_name)
        .await?;
    ...
    if let Some(existing_fn) = existing {
        ...
        if existing_fn.tool_deployment_type_id != tool_deployment_type_id
            || existing_fn.tool_group_deployment_type_id != tool_group_deployment_type_id
            || existing_fn.tool_group_id != tool_group_instance_id
        {
            return Err(CommonError::InvalidRequest {
                msg: format!(
                    "Function name '{}' already exists in MCP server instance '{}'",
    ...
    let update_params = UpdateMcpServerInstanceFunction {
        ...
        tool_group_id: tool_group_instance_id.to_string(),
        tool_name: request.tool_name.clone(),
        tool_description: request.tool_description.clone(),
    };

    repository
        .update_mcp_server_instance_tool(&update_params)
        .await?;
    ...
    if publish_on_change_evt {
        on_config_change_tx
            .send(OnConfigChangeEvt::McpServerInstanceToolUpdated(
                function_serialized,
            ))
            .map_err(|e| {
                CommonError::Unknown(anyhow::anyhow!("Failed to send config change event: {e}"))
...
/// Updates a function in an MCP server instance (only tool_name and tool_description)
#[allow(clippy::too_many_arguments)]
#[authz_role(Admin, permission = "mcp:write")]
#[authn]
pub async fn update_mcp_server_instance_tool<R: ProviderRepositoryLike>(
    on_config_change_tx: &OnConfigChangeTx,
    repository: &R,
    mcp_server_instance_id: &str,
    tool_deployment_type_id: &str,
    ...
    update_mcp_server_instance_tool_internal(
        on_config_change_tx,
        repository,
        mcp_server_instance_id,
...
pub async fn remove_mcp_server_instance_function_internal<R: ProviderRepositoryLike>(
    ...
    // Find the function in the instance
    let function_exists = instance.tools.iter().any(|f| {
        f.tool_deployment_type_id == tool_deployment_type_id
            && f.tool_group_deployment_type_id == tool_group_deployment_type_id
            && f.tool_group_id == tool_group_instance_id
    });
    ...
    repository
        .delete_mcp_server_instance_tool(
            mcp_server_instance_id,
            tool_deployment_type_id,
            tool_group_deployment_type_id,
            tool_group_instance_id,
    ...
    if publish_on_change_evt {
        on_config_change_tx
            .send(OnConfigChangeEvt::McpServerInstanceToolRemoved(
                mcp_server_instance_id.to_string(),
                tool_deployment_type_id.to_string(),
                tool_group_deployment_type_id.to_string(),
                tool_group_instance_id.to_string(),
            ))
            .map_err(|e| {
                CommonError::Unknown(anyhow::anyhow!("Failed to send config change event: {e}"))
            })?;
...
mod tests {
    mod unit {
        ...
        use crate::logic::instance::{ToolInstanceSerialized, ToolGroupInstanceSerialized};
        use crate::repository::{
            CreateToolInstance, CreateToolGroupInstance, CreateResourceServerCredential,
        ...
        async fn setup_tool_instance(
            ...
            let tool_group_deployment_type_id = "google_mail".to_string();
            ...
            repo.create_tool_group_instance(&CreateProviderInstance::from(tool_group_instance))
                .await
                .unwrap();

            // Create function instance
            let tool_deployment_type_id = "send_email".to_string();
            ...
            repo.create_tool_instance(&CreateToolInstance::from(tool_instance))
                .await
                .unwrap();
        ...
        async fn test_add_mcp_server_instance_function() {
            ...
            let (tool_deployment_type_id, tool_group_deployment_type_id, tool_group_instance_id) =
                setup_tool_instance(&repo).await;
            ...
            let add_request = AddMcpServerInstanceFunctionRequest {
                ...
                tool_group_deployment_type_id: tool_group_deployment_type_id.clone(),
                tool_group_id: tool_group_instance_id.clone(),
                tool_name: "my_custom_function".to_string(),
            ...
            assert_eq!(instance.tools[0].tool_name, "my_custom_function");
            assert_eq!(
        ...
        async fn test_add_mcp_server_instance_function_duplicate_name() {
            ...
            let add_request = AddMcpServerInstanceFunctionRequest {
                ...
                tool_group_deployment_type_id: tool_group_deployment_type_id.clone(),
                tool_group_id: tool_group_instance_id.clone(),
                tool_name: "duplicate_name".to_string(),
            ...
            let now = WrappedChronoDateTime::now();
            let tool_instance2 = ToolInstanceSerialized {
                tool_deployment_type_id: "another_function".to_string(),
                tool_group_deployment_type_id: tool_group_deployment_type_id.clone(),
                tool_group_id: tool_group_instance_id.clone(),
                created_at: now,
                updated_at: now,
            };
            repo.create_tool_instance(&CreateToolInstance::from(tool_instance2))
                .await
                .unwrap();
            ...
            let add_request2 = AddMcpServerInstanceFunctionRequest {
                ...
                tool_group_deployment_type_id: tool_group_deployment_type_id.clone(),
                tool_group_id: tool_group_instance_id.clone(),
                tool_name: "duplicate_name".to_string(), // Same name
                tool_description: None,
            };
        ...
        #[tokio::test]
        async fn test_add_mcp_server_instance_tool_instance_not_found() {
            shared::setup_test!();
            ...
            let add_request = AddMcpServerInstanceFunctionRequest {
                tool_deployment_type_id: "nonexistent".to_string(),
                tool_group_deployment_type_id: "nonexistent".to_string(),
                tool_group_id: "nonexistent".to_string(),
                tool_name: "my_function".to_string(),
                tool_description: None,
            };
        ...
        async fn test_update_mcp_server_instance_tool() {
            ...
            let result = update_mcp_server_instance_tool_internal(
                &tx,
                &repo,
                &create_request.id,
                &tool_deployment_type_id,
        ...
        #[tokio::test]
        async fn test_update_mcp_server_instance_tool_name_conflict() {
            shared::setup_test!();
            ...
            let add_request1 = AddMcpServerInstanceFunctionRequest {
                ...
                tool_group_deployment_type_id: tool_group_deployment_type_id.clone(),
                tool_group_id: tool_group_instance_id.clone(),
                tool_name: "first_function".to_string(),
                tool_description: None,
            };
            add_mcp_server_instance_function_internal(
                &tx,
            ...
            let add_request2 = AddMcpServerInstanceFunctionRequest {
                ...
                tool_group_deployment_type_id: tool_group_deployment_type_id.clone(),
                tool_group_id: tool_group_instance_id.clone(),
                tool_name: "second_function".to_string(),
                tool_description: None,
            };
            add_mcp_server_instance_function_internal(
                &tx,
            ...
            let update_request = UpdateMcpServerInstanceFunctionRequest {
                tool_name: "first_function".to_string(), // Conflict!
                tool_description: None,
            };
        ...
        #[tokio::test]
        async fn test_update_mcp_server_instance_tool_same_name_allowed() {
            shared::setup_test!();
            ...
            let update_request = UpdateMcpServerInstanceFunctionRequest {
                tool_name: "my_function".to_string(), // Same name
                tool_description: Some("New description".to_string()),
            };
            ...
            let instance = result.unwrap();
            assert_eq!(instance.tools[0].tool_name, "my_function");
            assert_eq!(
                instance.tools[0].tool_description,
        ...
        async fn test_remove_mcp_server_instance_function() {
            ...
            let add_request = AddMcpServerInstanceFunctionRequest {
                ...
                tool_group_deployment_type_id: tool_group_deployment_type_id.clone(),
                tool_group_id: tool_group_instance_id.clone(),
                tool_name: "to_be_removed".to_string(),
                tool_description: None,
            };
            add_mcp_server_instance_function_internal(
                &tx,
