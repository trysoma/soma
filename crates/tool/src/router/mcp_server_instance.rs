...
use crate::logic::{
    ...
    update_mcp_server_instance_tool,
};
...
#[utoipa::path(
    patch,
    path = format!("{}/{}/{}/mcp-server/{{mcp_server_instance_id}}/function/{{tool_deployment_type_id}}/{{tool_group_deployment_type_id}}/{{tool_group_instance_id}}", PATH_PREFIX, SERVICE_ROUTE_KEY, API_VERSION_1),
    tags = [SERVICE_ROUTE_KEY, API_VERSION_TAG],
    params(
        ("mcp_server_instance_id" = String, Path, description = "MCP server instance ID"),
        ...
        ("tool_group_instance_id" = String, Path, description = "Provider instance ID"),
    ),
...
pub async fn route_update_mcp_server_instance_tool(
    State(ctx): State<McpService>,
    headers: HeaderMap,
    Path((
        mcp_server_instance_id,
    ...
    let res = update_mcp_server_instance_tool(
        ctx.auth_client().clone(),
        headers,
        ctx.on_config_change_tx(),
        ctx.repository(),
