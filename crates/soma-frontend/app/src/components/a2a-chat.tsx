				...
				<ConversationContent>
					{messages.map(({ versions, ...message }) => {
						...
						const versionsToRender =
							versions.length > 0 ? versions : [{ id: "0", content: "" }];
...
function ListSidebar<T extends BaseItem>({
	list,
	setCurrent,
	currentId,
	mainButton,
	...
	return (
		<Sidebar className="sticky w-full" collapsible="icon">
			<SidebarHeader>
				<SidebarMenu>
					<SidebarMenuItem>
						<div className="flex data-[state=open]:bg-sidebar-accent data-[state=open]:text-sidebar-accent-foreground items-center justify-center">
							{state === "expanded" ? (
								<>
					...
					{state === "expanded" && (
						<SidebarMenuItem>
							<div className="grid flex-1">{mainButton}</div>
						</SidebarMenuItem>
					)}
					{list
