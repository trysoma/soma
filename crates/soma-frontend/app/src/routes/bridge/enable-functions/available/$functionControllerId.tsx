...
function RouteComponent() {
	const { functionControllerId } = useParams({
		from: "/bridge/enable-functions/available/$functionControllerId",
	});
	...
	const { func, provider } = useMemo(() => {
		...
		let foundProvider:
			| components["schemas"]["ProviderControllerSerialized"]
			| null = null;
	...
	const getCurrentTab = () => {
		if (location.pathname.includes("/function_documentation"))
			return "function";
		if (location.pathname.includes("/provider_documentation"))
			return "provider";
	...
	const tabs = [
		...
		},
		{
			...
			component: (
				<Link
					to={LINKS.BRIDGE_ENABLE_FUNCTIONS_CONFIGURE(functionControllerId)}
				>
					Configure
				</Link>
			),
			...
			? [
					{
						...
						component: (
							<Link
								to={LINKS.BRIDGE_ENABLE_FUNCTIONS_TEST(functionControllerId)}
							>
								Test
							</Link>
						),
	...
	return (
		...
		>
			<SlideOutTabs
				...
				className={hasEnabledInstances ? "grid-cols-4" : "grid-cols-3"}
			/>
		</SlideOutPanel>
	);
}
