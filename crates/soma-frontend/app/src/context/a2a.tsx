...
const mapA2aTaskToWrappedTask = (task: Task): WrappedTask => {
	return {
		...
		aiSdkView: {
			messages:
				task.history?.map((message) => mapA2aMessageToAiSdkMessage(message)) ??
				[],
		},
	};
};

const mapSomaPartToA2aPart = (
...
const mapSomaMessageToA2aMessage = (
...
const mapSomaMessageToAiSdkMessage = (
	message: components["schemas"]["Message"],
): MessageType => {
	return {
		key: message.id,
...
const mapSomaTaskToWrappedTask = (
	task: components["schemas"]["TaskWithDetails"]["task"],
	status_message?: components["schemas"]["TaskWithDetails"]["status_message"],
	messages?: components["schemas"]["TaskWithDetails"]["messages"],
): WrappedTask => {
	return {
		somaView: {
		...
		a2aView: {
			...
			status: {
				...
				message: status_message
					? mapSomaMessageToA2aMessage(status_message)
					: undefined,
		...
		aiSdkView: {
			messages:
				messages?.map((message) => mapSomaMessageToAiSdkMessage(message)) ?? [],
		},
	};
};

const mapA2aTaskStatusUpdateToWrappedTask = (
	...
	if (statusMessage?.parts) {
		messageContent = statusMessage.parts
			.map((part) => {
				if (part.kind === "text") {
					return part.text;
				}
...
function A2aProviderInner({ children }: { children: ReactNode }) {
	...
	const fetchPageOfContexts = async (pageToken: string | null) => {
		...
		setContexts((prev) => {
			...
			for (const newContext of newContexts) {
				const foundContext = prevCopy.find(
					(c) => c.id === newContext.context_id,
				);
	...
	const fetchPageOfTasks = async (
		...
		setContexts((prev) => {
			...
			for (const remoteTask of res.data.items) {
				const foundTask = cachedContext.tasks.find(
					(t) => t.somaView.id === remoteTask.id,
				);
				if (foundTask == null) {
					cachedContext.tasks.push(
						mapSomaTaskToWrappedTask(remoteTask, undefined, undefined),
					);
				}
			}
			...
			switch (event.kind) {
				...
				case "task": {
					...
					setContexts((prev) => {
						...
						const cachedTask = cachedContext.tasks.find(
							(t) => t.somaView.id === event.id,
						);
				...
				case "status-update": {
					setContexts((prev) => {
						...
						const prevCopy = [...prev];
						const cachedContext = prevCopy.find(
							(c) => c.id === event.contextId,
						);
						if (!cachedContext) {
							return prevCopy;
						}
						let cachedTask = cachedContext.tasks.find(
							(t) => t.somaView.id === event.taskId,
						);
						if (!cachedTask) {
							return prevCopy;
						}
						cachedTask = mapA2aTaskStatusUpdateToWrappedTask(
							cachedTask,
							event.status,
						);
				...
				case "message": {
					setContexts((prev) => {
						...
						const cachedContext = event.contextId
							? prevCopy.find((c) => c.id === event.contextId)
							: prevCopy.find((c) =>
									c.tasks.some((t) => t.somaView.id === event.taskId),
								);
						...
						const messageExists = cachedTask.a2aView.history.some(
						...
						const aiMessageExists = cachedTask.aiSdkView.messages.some(
							(msg) => msg.key === event.messageId,
						);
						if (aiMessageExists) {
							return prevCopy;
						}
	...
	const setCurrentTask = async (taskId: string | null) => {
		...
		if (taskId === null) {
			...
			const context = contexts.find(
				(context) => context.id === selectedIds.contextId,
			);
			if (!context) {
			...
			if (!cachedTask || cachedTask.aiSdkView.messages.length === 0) {
				...
				cachedTask = mapSomaTaskToWrappedTask(
					somaTask.task,
					somaTask.status_message,
					somaTask.messages,
				);
			}
			...
			setContexts((prev) => {
				...
				const cachedTaskIndex = cachedContext.tasks.findIndex(
					(t) => t.somaView.id === taskId,
				);
	...
	const sendMessage = async (message: string) => {
		...
		try {
			...
			const resGenerator = a2aClient.current.sendMessageStream({
				message: {
	...
	const currentContext =
		contexts.find((context) => context.id === selectedIds.contextId) ?? null;
	const currentTask =
		currentContext?.tasks.find(
			(task) => task.somaView.id === selectedIds.taskId,
		) ?? null;
	...
	return (
		...
		>
			{ready && agentCard && a2aClient.current ? (
				children
			) : (
				<div className="flex items-center justify-center h-screen">
					Loading...
