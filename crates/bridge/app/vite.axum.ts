...
function getRoutes(): string[] {
  const routes = (globalThis as any)
    .TSR_ROUTES_BY_ID_MAP as Map<string, { routePath: string }>;
  ...
  return {
    ...
    handleHotUpdate(ctx) {
      if (ctx.file.includes("/routes/")) {
        ...
        const outPath = path.resolve(ctx.server.config.root, outDir, OUTPUT_FILE_NAME);
