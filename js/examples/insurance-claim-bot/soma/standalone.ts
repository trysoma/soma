...
import {
	...
	setSecretHandler,
	setUnsetEnvironmentVariableHandler,
...
setSecretHandler(async (err, secrets) => {
	...
	console.log(
		`[INFO] Secret handler invoked with ${secrets.length} secrets: ${secretKeys.join(", ")}`,
	);
...
setEnvironmentVariableHandler(async (err, envVars) => {
	...
	console.log(
		`[INFO] Environment variable handler invoked with ${envVars.length} environment variables: ${envVarKeys.join(", ")}`,
	);
...
setUnsetEnvironmentVariableHandler(async (err, key) => {
	...
	console.log(
		`[INFO] Unset environment variable handler invoked with key: ${key}`,
	);
	delete process.env[key];
	console.log(`[INFO] Removed process.env.${key}`);
	const message = `Removed environment variable '${key}' from process.env`;
	console.log(
		`[INFO] Unset environment variable handler completed: ${message}`,
	);
...
// Register function: approveClaim
{
	...
	if (fn.functionController && fn.providerController && fn.handler) {
		const providerTypeId =
			fn.providerController.typeId || fn.providerController.typeId;
...
const waitForPortAvailable = async (
	...
	while (Date.now() - startTime < maxWaitSeconds * 1000) {
		...
		if (available) {
			if (attempt > 0) {
				console.log(
					`Port ${port} is now available after waiting for previous instance to shut down.`,
				);
			}
			return;
		}
	...
	throw new Error(
		`Port ${port} did not become available within ${maxWaitSeconds} seconds. Please check if another process is using the port.`,
	);
};
...
httpServer.listen(restatePort, async () => {
	...
	for (let attempt = 1; attempt <= maxRetries && !resyncSuccess; attempt++) {
		console.log(
			`Triggering resync with API server (attempt ${attempt}/${maxRetries})...`,
		);
		try {
			await resyncSdk();
			...
			if (attempt < maxRetries) {
				...
				console.error(
					"Failed to resync with API server after all retries:",
					error,
				);
