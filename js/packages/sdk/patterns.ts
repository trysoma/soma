...
export const chat = <Input, Output>(
	handler: (params: ChatHandlerParams<Input, Output>) => Promise<void>,
) => {
	return async ({
		ctx,
		soma,
...
export const workflow = <Input, Output>(
	handler: (params: WorkflowHandlerParams<Input, Output>) => Promise<Output>,
) => {
	return async ({
		ctx,
		soma,
